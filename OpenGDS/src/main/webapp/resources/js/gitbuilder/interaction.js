...
gitbuilder.interaction.MultiTransform.prototype.handleDownEvent = function (evt) {
	...
	if (feature) {
		...
		if (element.style.cursor !== '' && this.task_) {
			...
			this.flatInteriorPoint_ = feature.getGeometry().getFlatInteriorPoints();
		}
	}
...
gitbuilder.interaction.MultiTransform.prototype.handleDragEvent = function (evt) {
	...
	if (this.scale_ === true) {
		...
		var center = feature.getGeometry().getFlatInteriorPoints();

		if (magni[0] > magni[1]) {
			feature.getGeometry().scale(magni[0], magni[0], center);
		} else {
			feature.getGeometry().scale(magni[1], magni[1], center);
		}
	}

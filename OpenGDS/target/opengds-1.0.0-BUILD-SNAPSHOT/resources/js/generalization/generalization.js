...
//    $('#genLayerManageTableBody > tr').remove();
	...
	$('#genLayerManageTableBody').append(
		"<tr>" +
		"<td style='vertical-align: middle;'><div class='checkbox checkbox-primary checkbox-single' style='margin-top: 0px;margin-bottom: 0px;'><input type='checkbox' name='chklayer' id='singleCheckbox" + obj + "'><label></label></div></td>" +
		"<td style='vertical-align: middle;'>" + layerAddList[obj].geomType + "</td>" +
		"<td style='vertical-align: middle;'>" + layerAddList[obj].lName + "</td>" +
		"<td><button type='button' class='btn btn-primary' id='detaillayerbtn"+obj+"'>Detail Info</button></td>" +
		"</tr>");
	...
	if(layerAddList[obj].geomType=='MultiPolygon'||layerAddList[obj].geomType=='Polygon'){
        	$('#genOption').append(
        		...
        		"<div class='col-sm-10'>" +
        		"<input type='text' class='form-control' id='elival"+obj+"' placeholder='Area Value'>"+
        		"</div>"+
        		"</div>"
        		);
	}
...
function generalizationRun() {
	...
	if (layerInfo.geomType == 'MultiPolygon'|| layerInfo.geomType == 'Polygon') {
	...
	} else if (layerInfo.geomType == 'MultiLineString'|| layerInfo.geomType == 'LineString') {
	    var elival = $('#elival' + obj + '').val();
	    var simdistance = $('#simdistance' + obj + '').val();
	    layerJson = {
		geojson : geojson,
		type : layerInfo.geomType,
		layerName : layerInfo.lName,
...
//	 $('#genResult').remove();
	 ...
	 var topologyTable = result.topologyTable;
	 $('#topologyTableBody > tr').remove();
	 for(var obj in topologyTable){
	     var firstObjs = topologyTable[obj].firstObjs.toString().replace(/,/g,', ');
	     var lastObjs = topologyTable[obj].lastObjs.toString().replace(/,/g,', ');

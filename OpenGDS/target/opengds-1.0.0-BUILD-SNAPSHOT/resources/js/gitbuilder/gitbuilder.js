...
$(document).ready(
		function() {
			...
			$("#errContent").css("width", winSz[0]).css("height", 0).css(
					"background-color", "#ffffff").css("visibility", "hidden");
			...
			$("#openReport").on(
					"click",
					function() {
						$("#errContent").css("height", 600).css("visibility",
								"visible").position({
							at : "right bottom",
							my : "right bottom",
							of : "#map"
...
function applyLayerStyle(layer) {
	...
	if (layerType === "point" || layerType === "linestring"
			|| layerType === "polygon" || layerType === "multipoint"
		...
		// 점의 표시타입을 선택
		var selectedMarker = $("input[type='radio'][name='markerType']:checked")
				.val();
		...
		if ($("#pointType").val() === "marker" && selectedMarker !== undefined) {
			...
			markerStyle = new ol.style.Icon({
				anchor : [ 0.5, 1 ],
				src : getContextPath() + "/js/gitbuilder/image/marker/"
						+ selectedMarker
			});
...
function loadLayerStyle(layerId) {
	...
	} else if (layerType === "linestring" || layerType === "polygon"
			|| layerType === "multilinestring" || layerType === "multipolygon") {
						...
						stop : function(event, ui) {
							...
							for (var i = 0; i < layerLiArr.length; i++) {
								...
								for (var j = 0; j < addedLayers.length; j++) {
									...
									if (addedLayers[j].get("id") === $(
											layerLiArr[i]).attr("id")) {
										if (addedLayers[j] instanceof ol.layer.Group) {
											addedLayers[j].setZIndex(i);
											var layers = addedLayers[j]
													.getLayers().getArray();
						...
						stop : function(event, ui) {
							...
							selected.each(function() {
								...
								if (p1.get(0) === p2.get(0)) {
									...
									var selectedLayer = getLayerById($(this)
											.attr("id"));
									// 선택여부를 1로 설정
							...
							if (selected.length > 1) {
								...
								if (layer instanceof ol.layer.Image) {
									...
									for (var i = 0; i < layers.length; i++) {
										if (layers[i] instanceof ol.layer.Tile) {
											view.fit(layers[i].getExtent(), map
													.getSize());
										}
									}
								}
								...
								if (selected[0].get("type") === "point"
									...
									var layers = getSelectedLayers();
									if (layers.length === 1
											&& layers[0] instanceof ol.layer.Vector) {
										$("#featureList").empty();
										var features = layers[0].getSource()
												.getFeatures();
										for (var i = 0; i < features.length; i++) {
												...
												.selectable(
														{
															...
															stop : function(
																...
																if (selectedFeatures
																	...
																	popFeatureDialog(selectedFeatures
																			.getArray());
																	$(
																			"#selectPopUp")
																			.dialog(
																	...
																	if (selectedFeatures
																		...
																		var keys = feature
																				.getKeys();
																		var features1 = selected[0]
																				.getSource()
																				.getFeatures();
																		var keys = features1[0]
																				.getKeys();
																		var flag = true;
																		...
																		if (flag) {
																			...
																			var theFeature = getFeatureById2(
																					layerId,
																					featureId);
																			collFeatures
																					.push(theFeature);
																			updateSelectInteraction2(
																					layers,
																					collFeatures,
																					map);
																		...
																		if (flag) {
																			...
																			for (var i = 0; i < selectedFeatures
																					.getLength(); i++) {
																				var featureId = selectedFeatures
																						.item(
																								i)
																						.getId();
																				var layerId = featureId
...
function getUserSelectStyle() {
	...
	if (type === "point" || type === "multipoint") {
		...
		if ($("#pointType").val() === "marker" && selectedMarker !== undefined) {
			...
			// 선택한 스타일로 서클스타일 초기화
			vectorStyle = new ol.style.Style({
				image : makeOlStyleObj("circle", colorObjS, colorObjF, width,
						radius)
			});
		}
		// 라인스트링 레이어 생성시
...
function updateLayerList(map) {
	...
	for (var i = 0; i < addedLayers.length; i++) {
		...
		var layerKeysStr = "";
		layerKeysStr += "<li id='" + addedLayers[i].get("id")
				+ "' class='layerLi'><p class='ListedLayer'>"
				+ addedLayers[i].get("name")
				+ "</p><button class='layerPropBtn'>Properties</button></li>";

		$("#layerList").prepend(layerKeysStr);

	}
	$(".ListedLayer").css("display", "inline-block").css("width", 160).css(
			"height", 16).css("text-overflow", "ellipsis");
	$(".layerPropBtn").button({
		icons : {
...
function changeBtnFntn(cond) {
	...
	if (cond === "1") {
		...
		$("#layerName").prop("readonly", true).css("border", "none").css(
				"width", 300);
		...
		$("#ptType").prop("checked", true);
		$("#layerName").prop("readonly", false).css("border",
				"1px solid #e3e3e3").css("width", 350);
		...
		$("#layerStyle").dialog(
				{
					buttons : {
						"확인" : function() {
							var creatingLayer = $("#tabs").tabs("option",
									"active");
							...
							case 0:
								...
								layer = setLayerProperties(layer, name, id,
										style[0], "new", 1, true, undefined);
								addLayerOnList(layer);
								break;
							case 1:
								var type = $("#baseType").val();
								var layer = createBaseTileLayer(type);
								layer = setLayerProperties(layer, type
										+ " base " + id, id, "tile", "new", 1,
										false, null);
								addLayerOnList(layer);
								break;
							case 2:
...
function getAllVectorLayers(map) {
	...
	for (var i = 0; i < layers.length; i++) {
		if (layers[i] instanceof ol.layer.Vector
				&& (layers[i].get("type") === "point"
						|| layers[i].get("type") === "linestring"
						|| layers[i].get("type") === "polygon"
						|| layers[i].get("type") === "multipoint"
						|| layers[i].get("type") === "multilinestring" || layers[i]
		...
		draw.on('drawend', function(e) {
			...
			if (selected.length === 1) {
				...
				if (selected[0].get("type") === "point"
					...
					var str = '<li class="ui-widget-content" id="'
							+ feature.getId() + '" style="padding:3px;">'
							+ feature.getId() + '</li>';
					$("#featureList").append(str);
...
function addSelectInteraction() {
	...
	select.on("select", function() {
		if (selectedFeatures.getLength() > 0) {
			...
			var atPosition = "left+" + crsrPosition[0] + "px " + "top+"
					+ crsrPosition[1] + "px";
			...
			if (selectedFeatures.getLength() === 1) {
				...
				if (flag) {
					...
					var featureId = selectedFeatures.item(0)
							.get("errfeatureID");
					// var featureId =
					// "layer1.1";
					var layerId = featureId
	...
	dragBox.on('boxend', function(e) {
		...
		for (var i = 0; i < selectLayers.length; i++) {
			if (selectLayers[i] instanceof ol.layer.Vector) {
				selectLayers[i].getSource().forEachFeatureIntersectingExtent(
			...
			if (selectedFeatures.getLength() === 1) {
				...
				if (flag) {
					...
					for (var i = 0; i < selectedFeatures.getLength(); i++) {
						...
						var layerId = featureId.substring(0, featureId
								.indexOf("."));
						// console.log(layerId);
						layerId = layerId.trim();
						var layer = getLayerById(layerId);
					...
					for (var i = 0; i < selectedFeatures.getLength(); i++) {
						var featureId = selectedFeatures.item(i).get(
								"errfeatureID");
						var layerId = featureId.substring(0, featureId
								.indexOf("."));
						// console.log(layerId);
...
function getGeojsonFromFeatures() {
	...
	var gjson = new ol.format.GeoJSON().writeFeatures(selectedFeatures
			.getArray());
	// console.log(gjson);
	return gjson;
}
...
function getPropertiesFromSelected() {
	...
	for (var i = 0; i < layerKeys.length; i++) {
		...
		str += "<tr><td><span class='attrKey'>" + key + "</span></td><td>"
				+ type + "</td><td><input type='text' value='" + value
				+ "' id='" + layer.get("id") + "." + key
				+ "' class='attrVal'/></td></tr>";
	}
		...
		if (layers[0].get("cat") === 2) {
			...
			for (var i = 0; i < keys.length; i++) {
				if (keys[i] === "geometry") {
					...
					str += '<tr><td style="border: 1px solid #e3e3e3;">'
							+ keys[i]
			...
			for (var i = 0; i < lyrKeys.length; i++) {
				if (lyrKeys[i] === "geometry") {
					...
					str += '<tr><td style="border: 1px solid #e3e3e3;">'
							+ lyrKeys[i]
							+ '</td><td style="border: 1px solid #e3e3e3; max-width:120px; word-wrap:break-word;">';
					if (props[lyrKeys[i]] === undefined
							|| props[lyrKeys[i]] === null) {
						str += "";
					} else {
						str += props[lyrKeys[i]];
					}
...
function uploadRasterToServer(inputId) {
	...
	dragBox.on('boxend', function(e) {
		...
		layer = setLayerProperties(layer, "raster.png", id, "raster", "new", 1,
				false, null);
		addLayerOnList(layer);
		removeMyInteraction(map);
	});
...
function qaRequest(layers) {
	...
	var url = getContextPath() + "/qualityAssurance/qa.ajax";

	for (var i = 0; i < layers.length; i++) {

		if (layers[i] instanceof ol.layer.Vector) {

			if (layers[i].get("qaOption") === undefined
					|| layers[i].get("qaOption") === null) {
			...
			if (layers[i].get("type") === "point") {
				...
				prmt.ptLayers[layers[i].get("id")] = {
					feature : new ol.format.GeoJSON().writeFeatures(layers[i]
				...
				if (layer instanceof ol.layer.Vector
					...
					prmt.ptLayers[layer.get("id")] = {
						feature : new ol.format.GeoJSON().writeFeatures(layer
								.getSource().getFeatures()),
				...
				if (layer instanceof ol.layer.Vector
						&& layer.get("type") === "multilinestring") {
	...
	sendObjectRequest(
			...
			function(data) {
				...
				for (var i = 0; i < keys.length; i++) {
					if (keys[i] === "ptErrLayers") {
						if (data.ptErrLayers.features.length > 0) {
							var ptErrLayer = new ol.format.GeoJSON()
									.readFeatures(data.ptErrLayers);
							...
							id = createLayerId();
							ptLayer = setLayerProperties(ptLayer, name, id,
									"point", "new", 2, true, null);
						...
						if (data.lnErrLayers.features.length > 0) {
							var lnErrLayer = new ol.format.GeoJSON()
									.readFeatures(data.lnErrLayers);
							...
							name = "line_qa_result";
							id = createLayerId();
							lnLayer = setLayerProperties(lnLayer, name, id,
									"point", "new", 2, true, null);
						...
						if (data.pgErrLayers.features.length > 0) {
							var pgErrLayer = new ol.format.GeoJSON()
									.readFeatures(data.pgErrLayers);
							var polygonSource = new ol.source.Vector({
								wrapX : false
							});
							polygonSource.addFeatures(pgErrLayer);

							var pgLayer = createVectorLayer(polygonSource,
									style);
							name = "polygon_qa_result";
							id = createLayerId();
							pgLayer = setLayerProperties(pgLayer, name, id,
									"point", "new", 2, true, null);
						...
						if (data.mtptErrLayers.features.length > 0) {
							var mtptErrLayer = new ol.format.GeoJSON()
									.readFeatures(data.mtptErrLayers);
							var mtpointSource = new ol.source.Vector({
								wrapX : false
							});
							mtpointSource.addFeatures(mtptErrLayer);

							var mtptLayer = createVectorLayer(mtpointSource,
									style);
							name = "mt_point_qa_result";
							id = createLayerId();
							mtptLayer = setLayerProperties(mtptLayer, name, id,
									"point", "new", 2, true, null);
						...
						if (data.mtlnErrLayers.features.length > 0) {
							var mtlnErrLayer = new ol.format.GeoJSON()
									.readFeatures(data.mtlnErrLayers);
							var mtlineSource = new ol.source.Vector({
								wrapX : false
							});
							mtlineSource.addFeatures(mtlnErrLayer);

							var mtlnLayer = createVectorLayer(mtlineSource,
									style);
							name = "mt_line_qa_result";
							id = createLayerId();
							mtlnLayer = setLayerProperties(mtlnLayer, name, id,
									"point", "new", 2, true, null);
						...
						if (data.mtpgErrLayers.features.length > 0) {
							var mtpgErrLayer = new ol.format.GeoJSON()
									.readFeatures(data.mtpgErrLayers);
							var mtpolygonSource = new ol.source.Vector({
								wrapX : false
							});
							mtpolygonSource.addFeatures(mtpgErrLayer);

							var mtpgLayer = createVectorLayer(mtpolygonSource,
									style);
							name = "mt_polygon_qa_result";
							id = createLayerId();
							mtpgLayer = setLayerProperties(mtpgLayer, name, id,
									"point", "new", 2, true, null);
				...
				for ( var obj in errReport) {
					$('#tableBody').append(
							"<tr>" + "<td>" + errReport[obj].type + "</td>"
									+ "<td>" + errReport[obj].numOfItems
									+ "</td>" + "<td>"
									+ errReport[obj].numOfErr + "</td>"
									+ "<td>" + errReport[obj].ratioOfErr
									+ "</td>" + "<td>"
									+ errReport[obj].accuracy + "</td>"
									+ "<td>" + errReport[obj].weights + "</td>"
									+ "<td>" + errReport[obj].weightedValue
									+ "</td>" + "</tr>");
...
function approvalRequest() {
	...
	var layer = new ol.format.GeoJSON().writeFeatures(getLayerById(qaLayerName)
			.getSource().getFeatures(), {
		dataProjection : "EPSG:4326",
		featureProjection : "EPSG:3857"
	});
	...
	sendObjectRequest(getContextPath() + "/receipt/approvalRequest.ajax",
...
function fileUploadAjax(url) {
	...
	$.ajax({
		...
		success : function(data, textStatus, jqXHR) {
			...
			layer = setLayerProperties(layer, fileId, fileId, result.layerType
					.toLowerCase(), result.layerFrom, 1, true, null);
			var view = map.getView();
	...
	for (var i = 0; i < layers.length; i++) {
		if (layers[i] instanceof ol.layer.Vector) {
			if (layers[i].get("type") === "point") {
				...
				if (layers[i].get("qaOption") === null
						|| layers[i].get("qaOption") === undefined) {
					pt += "<span> [검수 옵션 미설정] </span>";
				}
				pt += "<button id='qa_" + layers[i].get("id")
						+ "' class='openQAOptBtn'>검수 옵션";
				pt += "</button>";
				pt += "</p>";
			} else if (layers[i].get("type") === "linestring") {
				...
				ln += "<button id='qa_" + layers[i].get("id")
						+ "' class='openQAOptBtn'>검수 옵션";
				ln += "</button>";
				...
				pg += "<button id='qa_" + layers[i].get("id")
						+ "' class='openQAOptBtn'>검수 옵션";
				pg += "</button>";
				...
				mtpt += "<button id='qa_" + layers[i].get("id")
						+ "' class='openQAOptBtn'>검수 옵션";
				mtpt += "</button>";
				...
				mtln += "<button id='qa_" + layers[i].get("id")
						+ "' class='openQAOptBtn'>검수 옵션";
				mtln += "</button>";
				...
				mtpg += "<button id='qa_" + layers[i].get("id")
						+ "' class='openQAOptBtn'>검수 옵션";
				mtpg += "</button>";
...
function setLayerButton(div) {
	var buttons = '<div style="display:table;width:inherit;text-align:left;">';
...
function setLayerList(div) {
	...
	layer += '<td colspan="2"><div id="iconTab"><span><input id="marker1" type="radio" name="markerType" value="map-marker.png" checked="checked" /><img class="markerImg" src="'
			+ getContextPath()
			+ '/resources/js/gitbuilder/image/marker/map-marker.png"></span></div></td>';
	layer += '</tr>';
	layer += '</table>';
	layer += '</div>';
	...
	// layer += '<li id="simplification">일반화</li>';
	layer += '</ul>';
	layer += '</div>';
	...
	// 이슬기 추가
	// 이슬기 추가
	layer += '<div id="genSettingDiv" title="일반화 설정">';
	layer += '<div class="cntSection">';
	layer += '<div class="tbList">';
	layer += '<colgroup><col width="40%" /><col width="40%" /><col width="20%" /></colgroup>';
	layer += '<table id="genSettingTab" class="boardList">';
	layer += '<thead><tr><th>종류</th><th>설정값</th><th>값</th></tr></thead>';
	layer += "<tr><td><label for='genSim'>Simplification</label></td><td><label for='genSimDis'>Distance</label></td><td><input type='text' class='text good' id='simple' style='width:50px;'></td><td></td></tr>";
	layer += '</table>';
	layer += '</div>';
	layer += '</div>';
	layer += '</div>';
			...
			.click(
					function() {
						...
						$("#lyrAttrList")
								.append(
										"<tr class='LyrAttrRow'>"
												+ "<td class='LyrKeyName' style='border: 1px solid #e3e3e3;'>"
												+ keyName
												+ "</td><td class='LyrKeyType' style='border: 1px solid #e3e3e3;'>"
												+ keyType + "</td>" + "</tr>");
					});

	// 레이어 설정 메뉴
	$("#layerSettingMenu").menu().menu("widget").find(".ui-menu").css("margin",
			0);
	...
	$("#fll").spectrum(
			{
				...
				palette : [
						...
						[ "rgb(230, 184, 175)", "rgb(244, 204, 204)",
								...
								"rgb(133, 32, 12)", "rgb(153, 0, 0)",
								"rgb(180, 95, 6)", "rgb(191, 144, 0)",
								"rgb(56, 118, 29)", "rgb(19, 79, 92)",
								"rgb(17, 85, 204)", "rgb(11, 83, 148)",
...
function setMapArea(div) {
	...
	var view = new ol.View({
		center : ol.proj.transform([ 71.433333, 51.166667 ], 'EPSG:4326',
				'EPSG:3857'),
		zoom : 4,
		minZoom : 2,
		maxZoom : 19,
	...
	var CustomControl = function(opt_options) {
		...
		var button = '<img src="' + getContextPath()
				+ '/resources/js/gitbuilder/image/up.png" id="openReport" />';
	...
	var map = new ol.Map({
		controls : ol.control.defaults({
			...
			({
				...
				[ scaleLineControl, zoomSlider, msPstn, overView, rotate,
						new CustomControl() ]),
		target : id,
		layers : [],
		view : view
	...
	map.on("moveend", function() {
		if ((apprvlLyr !== undefined && apprvlLyr !== null)
				&& apprvlLyr instanceof ol.layer.Vector) {
...
function setToolBox() {
	...
	$(document).on(
			...
			function() {
				...
				if (layer instanceof ol.layer.Vector
					...
					$("#layerSettingPopup").dialog("widget").find(
							".ui-dialog-content").css("padding", 0).css(
							"margin", 0).css("overflow", "visible");
					$("#layerSettingPopup").dialog("widget").css(
							"background-color", "rgba(0,0,0,0)").css("border",
	...
	// 이슬기
	$(document).on("click", "#simplification", function() {
		// 창 출력
		$("#genSettingDiv").dialog("open");
	});
...
function getAttributeFromLayer(_layer) {
	...
	if (attrObj !== undefined) {
		...
		for (var i = 0; i < keys.length; i++) {
			str += "<tr class='LyrAttrRow'><td class='LyrKeyName' style='border: 1px solid #e3e3e3;'>"
					+ keys[i]
					+ "</td><td class='LyrKeyType' style='border: 1px solid #e3e3e3;'>"
					+ attrObj[keys[i]] + "</td></tr>";
		}
		return str;
	}
}
...
function checkZoomGetFeature(apprvlLyr, lName) {
	...
	if (zoomLevel >= 18) {
		...
		if (apprvlLyr instanceof ol.layer.Vector) {
			// console.log("getin");
			var xtnt = map.getView().calculateExtent(map.getSize());
			var trnsXtnt = ol.proj.transformExtent(xtnt, "EPSG:3857",
					"EPSG:4326");
			var prmt = {
				service : "WFS",
				version : "1.0.0",
...
function getApprovalLayer(name, type) {
	...
	$.ajax({
		...
		success : function(data, textStatus, jqXHR) {
			...
			layer = setLayerProperties(layer, name, name, type, "new", 0,
					false, null);
...
function showAttributeForFix(layer) {
	...
	for (var i = 0; i < keys.length; i++) {
		str += "<tr>";
		str += "<td>" + (i + 1) + "</td>";
		str += "<td><input type='checkbox' class='RequiredAttribute' value='"
				+ keys[i] + "'/></td>";
		str += "<td>" + keys[i] + "</td>";
		str += "</tr>";
	}
 ...
 * @date 2016. 02
 */
function newSHPWindow() {
	if (CheckId("shpWindow")) {

		var shp = "";

		shp += "<div title='SHP' id='shpWindow'>";

		shp += '<form id="shpFormat" method="post" enctype="multipart/form-data" action="file/upShpFile.ajax">';
		shp += '<div><label for="shp">불러올 레이어의 SHP파일을 등록하세요.</label> <br/> <input type="file" name="shp" id="shp"></div>';
		shp += '<br>';
		shp += '<div><label for="shx">불러올 레이어의 SHX파일을 등록하세요.</label> <br/> <input type="file" name="shx" id="shx"></div>';
		shp += '<br>';
		shp += '<div><label for="dbf">불러올 레이어의 DBF파일을 등록하세요.</label> <br/> <input type="file" name="dbf" id="dbf"></div>';
		shp += "</form>";
	...
	$("#shpWindow").dialog("open");
}

/**
 * element id가 중복됬는지 검사한다.
 * 
 * @author yijun.so
 * @date 2016. 02
 * @return Object
 */
function CheckId(_eid) {
	var idCheck = $('[id="' + _eid + '"]');
	if (idCheck.length === 0) {
		return true;
	} else {
 ...
 * @date 2016. 02
 */
function newVectorWindow() {
	if (CheckId("vectorWindow")) {
		var layer = "";

		layer += '<div title="Vector" id="vectorWindow">';
		layer += '<div class="styleAttribute" id="typeRadio">';
		layer += '<span class="typeRadio"><input id="ptType" type="radio" name="lType"value="point" checked="checked" /><label for="ptType">점</label></span>';
		layer += '<span class="typeRadio"><input id="lsType" type="radio" name="lType" value="linestring" /><label for="lsType">선</label></span>';
		layer += '<span class="typeRadio"><input id="pgType" type="radio" name="lType" value="polygon" /><label for="pgType">면</label></span>';

...
gitbuilder.interaction.MultiTransform = function (opt_options) {
	
	var options = opt_options ? opt_options : {};
	
	ol.interaction.Pointer.call(this, {
		handleDownEvent: gitbuilder.interaction.MultiTransform.prototype.handleDownEvent, 
		handleDragEvent: gitbuilder.interaction.MultiTransform.prototype.handleDragEvent, 
		handleMoveEvent: gitbuilder.interaction.MultiTransform.prototype.handleMoveEvent, 
		handleUpEvent: gitbuilder.interaction.MultiTransform.prototype.handleUpEvent
	});
...
ol.inherits(gitbuilder.interaction.MultiTransform, ol.interaction.Pointer);

/**
 * map 객체에 있는 interaction 중에 ol.interaction.Select 객체를 찾아 비활성화
 * 시키거나 활성화 시킴
 * MultiTransform 버튼을 클릭하였을때 객체 선택이 풀리는 경우를 방지하기 위해 사용
 * @param {Boolean}
 * @this {gitbuilder.interaction.MultiTransform}
 */
gitbuilder.interaction.MultiTransform.prototype.setActiveSelect_ = function (bool) {
	var map = this.getMap();
 ...
 * @this {gitbuilder.interaction.MultiTransform}
 */
gitbuilder.interaction.MultiTransform.prototype.getActiveSelect_ = function () {
	var map = this.getMap();
	var bool = null;
	map.getInteractions().forEach(function (interaction) {
		if (interaction instanceof ol.interaction.Select) {
...
/**
 * down이벤트 발생시 우선적으로 select interaction의 active를 확인하고
 * false일 경우 true로 값을 설정해준다.
 * 선택된 피쳐가 있고 cursor가 버튼 위에 위치해 있다면 작업을 수행한다.
 * 수행해야할 작업이 rotate라면 {this.rotate_}를 true로 설정하고 scale이면
 * {this.scale_}을 true로 설정한다. flip일 경우의 이벤트는 select interaction의 click
 * 이벤트와 겹치기때문에 동시 실행되어 예기치 못한 상황이 발생할 수 있으므로 select active를 false로 설정한다.
 * @return {Boolean} true 반환시 drag squence 시작
 * @this {gitbuilder.interaction.MultiTransform}
 */
gitbuilder.interaction.MultiTransform.prototype.handleDownEvent = function (evt) {
 ...
 * @this {gitbuilder.interaction.MultiTransform}
 */
gitbuilder.interaction.MultiTransform.prototype.handleDragEvent = function (evt) {
...
/**
 * drag 실행 중에는 실행되지 않는다. {this.selectTask_} 함수를 통해 MultiTransform 버튼의
 * 위치를 인식하고 커서가 버튼위에 있을때 {this.task_}의 값과 마우스 커서 스타일 값을 변경한다.
 * @this {gitbuilder.interaction.MultiTransform}
 */
gitbuilder.interaction.MultiTransform.prototype.handleMoveEvent = function (evt) {
	if (!evt.dragging) {
		var map = evt.map
		var feature = this.features_.item(0);
		var element = evt.map.getTargetElement();
...
/**
 * {this.task_}가 flip 작업을 위한 값일때 flip을 실행한다.
 * {this.rotate_}와 {this.scale_}의 값이 true 일때 해당작업들에서
 * 설정되었던 멤버변수들을 모두 초기화시킨다.
 * @return {Boolean} false 반환시 drag squence 종료
 * @this {gitbuilder.interaction.MultiTransform}
 */
gitbuilder.interaction.MultiTransform.prototype.handleUpEvent = function (evt) {
	var feature = this.features_.item(0);
...
/**
 * 'flip', 'rotate', 'scale' 기능을 수행할 수 있는 이벤트 영역을 
 * {ol.style.Style}객체로 그려낸다.
 * @param {ol.render.Event}
 * @this {gitbuilder.interaction.MultiTransform}
 */
gitbuilder.interaction.MultiTransform.prototype.drawMbr = function (evt) {
	...
	if (this.features_.item(0) !== undefined) {
		...
		var fill = new ol.style.Fill({color: 'rgba(152,152,152,0.9)'});
 ...
 * @this {gitbuilder.interaction.MultiTransform}
 */
gitbuilder.interaction.MultiTransform.prototype.getFeatures = function () {
	return this.features_;
};

/**
 * style로 그려진 MultiTransform의 버튼들의 영역을 설정하고 cursor가 그 위치에
 * 있을때 해당버튼에 맞는 작업의 이름을 String으로 반환한다. 커서가 버튼 영역에 놓여있지 않다면 
 * null 값을 반환한다.
 * @param {ol.Map}
 * @param {ol.Feature}
 * @param {Array}
 ...
 * @this {gitbuilder.interaction.MultiTransform}
 */
gitbuilder.interaction.MultiTransform.prototype.selectTask_ = function (map, feature, cursor) {
...
/**
 * 피처 회전 알고리즘. 'pointerdrag' 이벤트 발생전의 커서 위치값과 이벤트 
 * 발생시의 커서값을 인자로 받아 선택된 피쳐의 중점을 기준으로 두 점 사이의 각도를 반환
 * @param {Array} 'pointerdrag'이벤트 발생전의 커서 위치값
 * @param {Array} 'pointerdrag'이벤트 발생시의 커서 위치값
 * @return {Float} 사이각을 radian 형식으로 반환
 * @this {gitbuilder.interaction.MultiTransform}
 */
gitbuilder.interaction.MultiTransform.prototype.rotateAlgorithm_ = function (prevCursorPosition, currentCursorPosition) {
	var feature = this.features_.item(0);
	var result = null;
	var extent = feature.getGeometry().getExtent();
...
/**
 * 피처 확대, 축소 알고리즘. 선택된 scale버튼의 이전 좌표값과 
 * pointer를 drag함으로서 변경된 커서의 좌표, 두 좌표값사이 길이의 배율값을 구하여 
 * 그 배율의 절대값을 리턴한다.
 * @param {ol.Feature}
 * @param {Array} drag를 통해 변경된 커서의 위치
 * @return {Array} 늘어난 x좌표, y좌표 배율의 절대값
 * @this {gitbuilder.interaction.MultiTransform}
 */
gitbuilder.interaction.MultiTransform.prototype.scaleAlgorithm_ = function (feature, currentCursorPoint) {
	var map = this.getMap();
	var extent = feature.getGeometry().getExtent();
 ...
 */
gitbuilder.interaction.MultiTransform.prototype.flipAlgorithm_ = function (feature, direction) {
...
gitbuilder.interaction.MeasureTip = function (opt_options) {	
	
	ol.interaction.Interaction.call(this, {
		handleEvent: gitbuilder.interaction.MeasureTip.prototype.handleEvent
	});
	
	var options = opt_options ? opt_options : {};
	
	/**
	 * MeasureTip을 적용시킬 feature들을 저장
	 * ol.interaction.Select 객체를 저장하면 선택된 feature만 적용시킬 수 있음
	 * @type {ol.Collection.<ol.Feature>}
	 * @private
	 */
	this.features_ = options.features;
    ...
    /**
     * 이벤트 바인딩은 한번만 시행되어야 하므로 이벤트 바인딩을 하였을때
     * 이 멤버변수를 true설정해주어 이미 이벤트 바인딩이 이루어졌음을 
     * handleEvent함수에 알려준다
     * @type {Boolean}
     * @private
     */
    ...
    /**
     * measure 수치를 map에 나타내어줄 element와 overlay를 생성한다
     * 두 개를 객체형태로 저장하여 반환한다
     * @type {Function}
     * @return {Object}
     * @private
     */
    ...
    /**
     * 인자값으로 <ol.Feature>가 주어진다면 해당 feature에 관련된
     * element와 overlay만 삭제하고 인자값이 주어지지 않는다면
     * this.measureTipElement_와 this.measureTipOverlay_ 멤버변수에
     * 저장되어 있는 모든 객체 또는 요소가 삭제된다.
     * @type {Function}
     * @private
     */
    ...
    /**
     * 인자값으로 주어진 라인스트링 객체에 대한 길이를
     * 계산하여 String 형식으로 반환한다
     * MultiLineString 객체일때에는 getLength() 함수가 적용되지 않으므로
     * 우선 LineString객체들을 추출해낸 다음 각 LineString객체들의 길이를 더하여
     * 반환한다
     * @type {Function}
     * @return {String}
     * @private
...
ol.inherits(gitbuilder.interaction.MeasureTip, ol.interaction.Interaction);

/**
 * 모든 마우스 이벤트 발생시에 실행되는 함수. this.features_ 멤버변수가 존재한다면
 * add이벤트와 remove이벤트를 추가로 바인딩 시켜주며 add 이벤트 발생시에는 선택된
 * feature에 대한 measure tip을 생성하고 remove 이벤트 발생시에는 tip을 삭제한다
 * 또한 ol.Collection<ol.interaction> 객체에도 remove 이벤트를 바인딩하여
 * gitbuilder.interaction.MeasureTip 객체가 삭제되었을때 모든 element와 
 * overlay들이 삭제되도록 하고 모든 이벤트 리스너를 해제한다.
 * @param {ol.MapBrowserEvent}
 * @return {Boolean} true가 반환되면 다른 함수들에도 전파된다. false가 반환되면
 * 전파가 중지된다.
 */
gitbuilder.interaction.MeasureTip.prototype.handleEvent = function (evt) {
	var that = this;
	var map = evt.map;
	...
	if (!this.constructor_) {
		...
		listen = interactions.on('remove', function (evt) {
			if (evt.element instanceof gitbuilder.interaction.MeasureTip) {
...
/**
 * ol.interaction.Draw객체의 drawStart 이벤트 발생시 실행되는 함수
 * 내부에 적어넣으면 현재 그리고 있는 feature에 대한 면적 또는 길이를 보여준다
 * @param {ol.interaction.Draw.Event}
 */
gitbuilder.interaction.MeasureTip.prototype.drawStart = function (evt) {
	var that = this;
...
/**
 * ol.interaction.Draw객체의 drawEnd 이벤트 발생시 실행되는 함수
 * 내부에 적어넣으면 생성된 feature에 대한 면적 또는 길이를 해당 객체에 고정시킨다
 * @param {ol.interaction.Draw.Event}
 */
gitbuilder.interaction.MeasureTip.prototype.drawEnd = function (evt) {
 ...
 * @param {ol.Feature}
 */
gitbuilder.interaction.MeasureTip.prototype.selectedMeasure = function (feature) {
	var output;
	var tipCoord;

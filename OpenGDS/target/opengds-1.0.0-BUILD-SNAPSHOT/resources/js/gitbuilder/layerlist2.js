...
if (!gitbuilder.ui)
	gitbuilder.ui = {};
gitbuilder.ui.LayerList = function LayerList(param) {
	this.map = param.map;
	...
	this.eid = "layerList";
	var count = 0;
	while (this.isDuplicatedID(this.eid)) {
		this.eid += count;
	}

	var list = $('<ul id="' + this.eid + '" class=".GitBuilder-LayerList"></ul>');
	$("#" + this.target).append(list);

	$("#" + this.eid).sortable({
		axis : "y",
		handle : ".GitBuilder-LayerList-Item-Handle"
	}).addClass("Gitbuilder-LayerSelectable").selectable({
		filter : "li",
	...
	$("#" + this.eid).on("selectablestop", function(event, ui) {
		// 선택한 li들의 객체배열을 획득
		var selected = $(".Gitbuilder-LayerSelectable .ui-selected");

		selectedLayers = new ol.Collection();
		// 각각의 li마다
		selected.each(function() {
			var layer = getLayerByName($(this).attr("layerName"));
	...
	// $("#" + this.eid).on("sortstop", function(event, ui, eid) {
	// // 선택한 li의 배열
	// var layerLiArr = $("#" + this.eid + " li").toArray();
	// // 배열을 뒤집는다
	// layerLiArr.reverse();
	// // 모든 레이어 획득
	...
	// if (addedLayers[j].get("name") === $(layerLiArr[i]).attr("name")) {
	...
	// var layer =
	// gitbuilder.method.layer.getLayerById($(this).attr("layerName"));
	...
	// });
	//
	// $(document).on("click", ".GitBuilder-LayerList-Function-Remove",
	// function() {
	// console.log($(this).attr("layerName"));
	// gitbuilder.method.layer.RemoveLayerOnList($(this).attr("layerName"));
	// });

	this.update();
};

gitbuilder.ui.LayerList.prototype = {
	constructor : gitbuilder.ui.LayerList,
	isDuplicatedID : function(eid) {
		var idCheck = $('[id="' + eid + '"]');
		if (idCheck.length === 0) {
			return false;
		} else {
	...
	add : function(obj) {
		...
		// attrType) {
		/*
		 * { layer: 레이어 객체 name: 레이어 이름 id: 레이어 아이디 type: 레이어 타입 - 폴리곤 error:
		 * 레이어 종류 - true: error/ false: normal edit: 수정가능 - 불린 attrType: 피처 속성
		 * {키: 데이터 타입, ...} }
		 */
		// 레이어의 갯수만큼의 인덱스를 부여(새로만든 레이어는 최상위로 보여진다)
		obj.layer.setZIndex(this.map.getLayers().getLength());
		...
		if (this.getLayerByName(obj.name)) {
			console.error("already existing name");
			return;
		} else {
	...
	update : function() {
		...
		for (var i = 0; i < addedLayers.length; i++) {
			...
			var layerKeysStr = "";
			layerKeysStr += "<li layerName='" + addedLayers[i].get("name") + "' class='GitBuilder-LayerList-Item'>";
			layerKeysStr += "<div>";

			layerKeysStr += addedLayers[i].get("name");

			// var legend =
			// gitbuilder.method.layer.CreateLegend(addedLayers[i]);
			// if (legend) {
			...
			layerKeysStr += '<button type="button" class="btn btn-default GitBuilder-LayerList-Function-Button GitBuilder-LayerList-Function-Show" layerName="'
			...
			layerKeysStr += '</button>';
			layerKeysStr += '<button type="button" class="btn btn-default GitBuilder-LayerList-Function-Button GitBuilder-LayerList-Function-Fit" layerName="'
					+ addedLayers[i].get("name") + '">';
			layerKeysStr += '<span class="glyphicon glyphicon-fullscreen" aria-hidden="true"></span>';
			layerKeysStr += '</button>';
			layerKeysStr += '<button type="button" class="btn btn-default GitBuilder-LayerList-Function-Button GitBuilder-LayerList-Function-Setting" layerName="'
					+ addedLayers[i].get("name") + '">';
			layerKeysStr += '<span class="glyphicon glyphicon-text-size" aria-hidden="true"></span>';
			layerKeysStr += '</button>';
			layerKeysStr += '<button type="button" class="btn btn-default GitBuilder-LayerList-Function-Button GitBuilder-LayerList-Function-Field" layerName="'
					+ addedLayers[i].get("name") + '">';
			layerKeysStr += '<span class="glyphicon glyphicon-list-alt" aria-hidden="true"></span>';
			layerKeysStr += '</button>';
			layerKeysStr += '<button type="button" class="btn btn-default GitBuilder-LayerList-Function-Button GitBuilder-LayerList-Function-Remove" layerName="'
					+ addedLayers[i].get("name") + '">';
...
gitbuilder.ui.LayerListWidget = $.widget( "ui.layerselectable", $.ui.mouse, {
	version: "1.12.1",
	options: {
		appendTo: "body",
		autoRefresh: true,
